...
- **Dataset canaries (MEASURED, Task 4 complete):** 451 restaurants, **378 brands**, **53 multi-location**, 16,402 active products.
  - The plan originally predicted 383/52 from a throwaway prototype. That was wrong twice over: 383 was arithmetic (`331 solo + 52 multi`) never checked against a real count, and the prototype's actual count was **381**. Reconciled exactly: 381 − 3 merge events − 0 splits = **378**.
  - The 3 merges are prototype bugs the real normalizer fixes, now locked by regression tests: `Gloria Jean's Coffee` and `Ledor` (prototype left a trailing outlet digit, e.g. `ledor 2`), and `Tabaq Coffee` (prototype stripped `gulshan` before `gulshan avenue`, stranding `avenue`).
