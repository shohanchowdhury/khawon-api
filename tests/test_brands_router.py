"""Brand page + brand dish detail, served under /restaurants/{slug}.
...
def test_brand_page_404s_for_unknown_brand(temp_db, db_session):
    from fastapi.testclient import TestClient
    from main import app
    assert TestClient(app).get("/restaurants/no-such-brand").status_code == 404
...
def test_brand_dish_detail_shows_per_branch_breakdown(temp_db, db_session):
    ...
    body = TestClient(app).get(f"/restaurants/{chain.chain_code}/dishes/{ft.id}/margherita").json()
...
def test_brand_dish_detail_404s_for_unknown_slug(temp_db, db_session):
    ...
    from main import app
    chain, ft = _seed_brand(db_session)
    assert TestClient(app).get(f"/restaurants/{chain.chain_code}/dishes/{ft.id}/nope").status_code == 404
